...
public class PlayerClimbController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        float inputY = KeybindingManager.GetMoveInputForTag(_playerTag).y;
        if (_canClimb && Mathf.Abs(inputY) > 0.01f)
        {
            _isClimbing = true;
        ...
        if (_isClimbing)
        {
            ...
            if (canExitByJump && KeybindingManager.GetJumpDownForTag(_playerTag))
            {
                StopClimbing();
            }
        }
